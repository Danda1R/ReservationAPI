...
export default class EventCard extends Component {
    ...
    dateToString = (dateString) =>{
        ...
        return date.getMonth()+"/"+date.getDate()+"/"+date.getFullYear()+" - "+(date.getHours()%12==0?12:date.getHours()%12)+":"+minutes+" "+merid
    }

    render = () =>{
        return(
            <TouchableOpacity style={{shadowColor:"#333",shadowOpacity:0.7,shadowRadius:5,shadowOffset:{width:3,height:1}}} onPress={()=>this.props.onPress(this.props.event)}>
                <View style={this.props.read? cardStyle.rviewbox : cardStyle.uviewbox}>
                    <View style={{justifyContent:'center', alignSelf:'center', marginRight:10}}>
                        ...
                        <Text numberOfLines={1} ellipsizeMode='tail' style={cardStyle.sender}>{this.props.senderName}</Text>
                    </View>
                    <View>
                        <Text numberOfLines={1} ellipsizeMode='tail' style={cardStyle.event}>{this.props.eventName}</Text>
                        <Text numberOfLines={3} ellipsizeMode='tail' style={cardStyle.loc}>{this.props.eventLoc}</Text>
                        <Text  style={cardStyle.date}>{this.dateToString(this.props.date)}</Text>
                    </View>
                </View>
            </TouchableOpacity>
        );
