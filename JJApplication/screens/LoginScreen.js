...
export default class LoginScreen extends Component{
        ...
        this.checkError()
         ...
         const {username, password} = this.state;
             try {
                ...
                this.props.navigation.replace("Home",{userID:user.data.listUsers.items[0].id});
             }
             } catch (error) {
    ...
    render = () =>{
        return(
            <View>
                <Text style={{fontSize:45, fontWeight:'bold',alignSelf:'center',marginTop:hp2dp("10")}}>Login</Text>
                <View style={{marginBottom:hp2dp("6")}}/>
                    <Textbox  label='Username' placeholder='Enter Username' error={this.state.username_error} inputHandler={this.setUsername}/>
                    <Textbox  label='Password' placeholder='Enter Password' error={this.state.password_error} secureTextEntry={true} inputHandler={this.setPassword}/>
                ...
                <TouchableOpacity  onPress={() => this.props.navigation.replace('Registration')}>
                    <Text style={{alignSelf:'center',marginTop:hp2dp("3%"), color:"#0AF",fontSize:18}}>Register</Text>
                </TouchableOpacity>
            </View>
        )
    }
