    ...
    const updatedRSVP = await API.graphql({
      ...
      variables: {
        input: {
          ...
          status: stat,
          _version: myRsvp.data.getRsvp._version,
        },
      },
    });
  };
        ...
        >
          <View key="1" style={{ backgroundColor: "#FFF" }}>
            ...
            >
              <Image
                source={{
                  uri: "https://github.com/Danda1R/ReservationAPI/blob/main/JJApplication/Spider-Verse%20%2312_0d21ebe8-5399-4933-8d41-fbb4230dee50.png",
                }}
                style={{
                  height: hp2dp("8%"),
                  width: hp2dp("8%"),
                  borderRadius: hp2dp("8%"),
                  borderWidth: 1,
                }}
              />
              <Text
            ...
            >
              Start: {this.dateToString(this.state.startDate)}
            </Text>
            <Text
              style={{
                fontSize: 20,
            ...
            >
              End: {this.dateToString(this.state.endDate)}
            </Text>
            <View
              style={{ marginTop: hp2dp("4%"), marginHorizontal: wp2dp("33%") }}
            >
              {false ? <SubmitButton title="Edit Event" /> : null}
            </View>
          </View>
          <View key="2">
            <GestureHandlerRootView style={{ flex: 1 }}>
              <DraxProvider style={{ backgroundColor: "#FFF", flex: 1 }}>
                ...
                {this.state.isHost ? (
                  <View style={{ marginVertical: hp2dp("2%") }}></View>
                ) : (
                  <DraxView
                    ...
                    renderContent={() => (
                      <ResponseCard
                        isUsers={true}
                        isHost={this.determineHost(
                          this.state.eventInfo.organizerid,
                          "31"
                        )}
                        profileSource="yellow"
                        username="This User"
                        responseSource={this.state.status}
                      />
                    )}
                ...
                <ScrollView style={{ maxHeight: hp2dp("53%") }}>
                  {this.state.guests.map((guest) => (
                    <ResponseCard
                      ...
                      key={guest.id}
                      profileSource={guest.profilePic}
                      username={guest.username}
                      responseSource={guest.response}
                    />
                  ))}
                </ScrollView>
